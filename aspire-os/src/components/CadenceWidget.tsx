...
export default function CadenceWidget({
  ...
  // Restore today's Cadence from cache immediately on mount —
  // so the protocol is visible the moment the dashboard loads.
  useEffect(() => {
    const cached = loadCache(todayKey());
    if (cached) {
      ...
      autoTriggeredRef.current = true; // skip auto-generate, cache is enough
    }
